...
-- 자체는 어떤 shop_items row에도 걸 수 있게 범용으로 둠(추후 일반 상품에도 열어줄 수 있게).
--
-- stock_limit: null이면 무제한(기존 상품 전부 이 상태). 값이 있으면 그 개수만큼만 판매 가능.
-- sold_count: 지금까지 이 상품이 팔린(사용된) 횟수. spend-points가 구매 성공마다 1씩 늘림.
-- 다 팔렸다고 상품을 지우거나 비활성화하는 게 아니라 "품절" 상태로만 표시하고 구매만 막음
