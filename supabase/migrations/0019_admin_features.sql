...
-- 놓쳤을 때 포인트 로그에서 대조하면서 "이미 처리했다"고 표시해둘 수 있게 함. 지급/차감/출석체크
...
-- (0001_init.sql 참고) 랭킹 화면이 points_ledger 변경을 직접 구독할 수 없음. 대신 이 작은 공개
-- 테이블에 트리거로 "핑"만 남기고, 프론트엔드는 이 테이블의 INSERT만 구독해서 핑이 오면 랭킹을
-- 다시 조회함 (핑 행 자체엔 아무 정보도 안 담음 — 그냥 "뭔가 바뀜" 신호용. overlay가 spend_events를
-- 구독하는 것과 같은 패턴 — 0003_realtime_spend_events.sql 참고).
-- 참고: points_ledger가 느는 속도만큼 이 테이블도 계속 늘어남 — points_ledger 자체도 지금 정리
-- 안 하고 있으니(잔액 계산 근거라 지울 수도 없음) 일단 같은 정책으로 둠. 나중에 너무 커지면
