...
-- 상품명을 같이 기록하게 한다 — 나중에 shop_items.name이 바뀌어도 과거 기록은 그대로 남음
...
-- 직접 호출 가능한 상태였다(Supabase 보안 점검에서 WARN). 트리거 안에서만 쓰는 new 레코드를
-- 참조하는 함수라 직접 호출하면 에러로 끝나긴 하지만, 트리거 실행 자체엔 이 권한이 필요 없으니
-- (트리거는 테이블 소유자 권한으로 도니까) 그냥 막아둔다.
...
-- 성능 점검에서 걸림 — 지금 유저 수에선 체감 차이 없지만 미리 걸어둔다.
