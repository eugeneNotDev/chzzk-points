...
--    잠금해제됨. 기존 성취 방식(users.max_balance_reached, 0016_titles.sql)과는 별개의
--    경로 — 둘 중 하나만 만족해도 그 칭호는 unlocked로 취급함. 완전히 구매 전용 칭호를
...
--    이 상품을 사용했을 때 spend_events에 기록을 안 남겨서 오버레이(overlay.html)에 안 뜸.
--    칭호 구매처럼 "방송 화면에 굳이 안 떠도 되는" 상품에 체크 해제해서 쓰라고 만듦.
-- 3) user_purchased_titles: 유저가 "구매"로 잠금해제한 칭호 기록. 성취 방식과 별개 경로라
--    따로 테이블로 둠 — 한 유저가 같은 칭호를 두 번 살 수 없게 (channel_id, title_id)를
--    기본키로 묶어서 막음.
...
-- 이 테이블은 Edge Function(서비스 롤)만 읽고 씀 — me(본인 소유 칭호 조회), spend-points
-- (구매 기록). shop_items/titles와 달리 프론트가 anon으로 직접 읽을 일이 없고(개인별 구매
-- 내역이라 공개할 이유가 없음), RLS는 켜두되 공개 정책은 열지 않음(서비스 롤은 RLS를
