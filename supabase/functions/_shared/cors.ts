...
// 프론트엔드(GitHub Pages) 도메인에서 오는 요청만 허용함.
// 로컬에서 테스트할 때는 ALLOWED_ORIGIN을 환경변수로 덮어써서 쓸 수 있게 해둠.

// 브라우저는 Origin 헤더의 호스트를 항상 소문자로 보냄 (URL 스펙상 host 정규화).
// 여기 값이 대소문자 하나라도 다르면 브라우저가 CORS 검증에서 실제 요청을 막아버림
...
// preflight(OPTIONS) 요청이면 바로 응답을 리턴하고, 아니면 null을 리턴함.
// 각 함수의 Deno.serve 맨 앞에서 이렇게 씀:
