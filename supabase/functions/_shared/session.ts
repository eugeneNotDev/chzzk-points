// 자체 세션 토큰(JWT) 발급/검증.
//
// 이 프로젝트는 Supabase Auth를 쓰지 않음 — 로그인 주체가 "치지직 채널"이라서,
// 치지직 OAuth로 확인한 channelId를 우리가 직접 서명한 토큰에 담아 프론트엔드에 내려줌.
// 프론트엔드는 이 토큰을 localStorage에 저장하고, 이후 모든 Edge Function 호출에
// `Authorization: Bearer <token>` 헤더로 붙여서 보냄 (docs/assets/js/chzzk-auth.js 참고).
//
// 서명 비밀키(SESSION_JWT_SECRET)는 이 함수들의 환경변수로만 존재함 (Supabase 프로젝트 설정에서 등록).
// 치지직 자체 accessToken/refreshToken과는 별개의, 우리 서비스 전용 세션 토큰임.
//
// 만료는 7일로 잡음. 만료되면 프론트엔드가 401을 받고 로그아웃 처리 후 재로그인 유도.
// 별도 리프레시 토큰 플로우(새 엔드포인트/새 토큰 종류)는 여전히 안 만듦 — 대신 아래
// shouldRefresh()로 "슬라이딩 세션"만 구현함: 유효기간이 얼마 안 남은 토큰으로 /me를 부르면
// 그 응답에 새 토큰을 얹어서 돌려주고, 프론트가 그걸 받아 localStorage를 갈아끼움
// (me/index.ts, docs/assets/js/chzzk-auth.js의 authFetch 참고). 새 테이블이나 DB 조회 없이
// JWT 서명 한 번 더 하는 것뿐이라 서버 부담이 거의 없음.

import { create, verify, getNumericDate, type Header, type Payload } from "https://deno.land/x/djwt@v3.0.2/mod.ts";

// issueSessionToken에 넘기는 입력값 — 발급 시점엔 만료 시각을 우리가 직접 정하므로 exp가 없음.
...
export interface SessionPayload extends IssuePayload {
  // 토큰 만료 시각(초 단위 유닉스 타임스탬프). 슬라이딩 세션 갱신(아래 shouldRefresh)에 씀.
  exp: number;
}

const SESSION_TTL_SECONDS = 60 * 60 * 24 * 7; // 7일

// 이 값보다 남은 유효기간이 적으면 새 토큰을 발급해줌("슬라이딩 세션" — 리프레시 토큰을
...
// 걸림. DB 조회나 새 테이블 없이 JWT 서명만 한 번 더 하는 거라 서버 부담은 거의 없음 —
// 로그인 자체가 뜸한 유저는 그냥 7일 뒤 만료되고(기존과 동일), 며칠에 한 번이라도 사이트에
// 들르는 유저는 사실상 로그인이 계속 유지됨.
...
// channelId/channelName을 담은 서명된 토큰 문자열을 만듦.
...
// 토큰을 검증하고 payload를 리턴함. 서명이 틀리거나 만료됐거나 형식이 이상하면 null.
...
// req의 Authorization 헤더(`Bearer <token>`)에서 토큰 문자열만 꺼냄. 없으면 null.
...
// spend-points, attendance-check 등 "로그인 필수" 함수들이 맨 앞에서 이렇게 쓰면 됨:
