...
- **칭호** — 최고 보유 포인트 기준으로 자동으로 붙는 등급 칭호(브론즈~다이아몬드)는 이름 색으로 보이고, 상점에서 산 칭호나 관리자가 직접 준 칭호는 배지로 보임. 배지는 색(예시 색/HEX 직접 입력/무지개, 두 가지 색 그라데이션이나 배경색+글자색 조합도 가능) + 디자인(기본/메탈 반짝/빛나는 테두리/유리) + 장식(왕관, 반짝이 별)을 골라서 꾸밈. 전부 서로 같이 쓸 수 있음(무지개만 두 번째 색이랑은 같이 안 됨). 마이페이지에서 골라서 장착함
