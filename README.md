...
- **출석체크** — 방송 중일 때만 하루 한 번 가능, 1회 50P. 달력으로 출석한 날 표시됨
- **포인트 상점** — 일반 상품(쿨타임, 방송 중 전용, 한정 수량 설정 가능)이랑 칭호 상품 두 종류 있음. 상품을 사면 OBS 오버레이에 알림이 뜸
- **칭호** — 최고 보유 포인트 기준으로 자동으로 붙는 등급 칭호(브론즈~다이아몬드)가 있고, 상점에서 사는 구매형 칭호는 마이페이지에서 골라서 장착함
...
- **관리자 페이지** — 유저 검색, 포인트 지급/차감(일괄 가능, 실행취소 가능), 밴, 전체 포인트 로그, 상점 사용 내역 처리
- **OBS 오버레이** — 상점 사용을 실시간으로 받아서 방송 화면에 알림 띄움

PC에선 왼쪽 사이드바, 폰에선 상단바 + 하단 탭바 레이아웃으로 바뀜.
...
- **포인트 잔액은 따로 저장 안 함.** `points_ledger`에 +/- 기록을 쌓고 합계로 계산함. 그래서 관리자 지급/차감도 지우는 대신 반대 기록을 추가하는 방식으로 실행취소함
